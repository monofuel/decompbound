...
| `$97F5` | `$4E0` | **Battery-save persist block** (maps to slot data `+$20`) | ✅ | Slot↔WRAM: money slot+`$5C`→`$9831`, roster+`$B6`→`$988B`, char0+`$1F9`→`$99CE`. Save-to-SRAM copies this block. |
| `$99CE` | `$5F`×4+ | **Party character table** (Ness, Paula, Jeff, Poo; same layout as SRAM char rows) | ✅ | Name+0, level+5, maxHP+`$0A`, maxPP+`$0C`, curHP+`$47`, curPP+`$4D`. `party_wram.nim` / Goal 5 live MCP. Cross-check: policy `PartyName0Wram`; `$4DC8`→`$99CE`; ROM `ADC #$99CE`. |
