...
| `+$19` | Type / equip-flag byte (soft) |
| `+$1A` | Price, u16 LE |
| `+$1C`… | More equip fields — still soft |

`src/decompbound/item_table.nim` decodes names at runtime from the user's ROM
(never baked into source — copyright). Example IDs: `0x11` Cracked bat,
`0x1A` Gutsy bat, `0xB1` ATM card.
