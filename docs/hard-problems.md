...
2. **Static DMA-path review (done 2026-07-07) — leading candidate: VMAIN address
   translation (`$2115` bits 3-2).** ~~Not implemented~~ → **implemented again**
   (`translatedVramAddr` on `$2118/$2119`; tests in `tests/test_vmain.nim`). Was briefly
   landed as `acee3ec` then reverted during audio stabilize; re-landed after
   Mr. Saturn dialogue left a hung black screen (`PC=00:5FFF`, VRAM scrambled).
   **Note:** a glitch F12 already embeds *corrupt* VRAM — the fix only helps *new*
   event DMAs. Reload a pre-event save and re-trigger Mr. Saturn / inn / sanctuary
   to verify live. Secondary candidates remain: wrong step (32/128 vs 1) or
   mode-1 word-pair split if garbage persists.
