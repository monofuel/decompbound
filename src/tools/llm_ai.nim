...
SETTING: You are Ness. A METEOR just crashed into the hills above Onett in the middle of the night — the whole town's rattled. This is EarthBound: it's funny, it's weird, lean into it. FOLLOW >>> CURRENT_OBJECTIVE at the top of the RICH STATE — it's authoritative and ADVANCES as you finish milestones, so never keep re-doing a finished one (once pokey_pct=100, STOP talking to Pokey and move on). The story ladder, each step unlocking the next:
  ...
  2. GET TO THE CRATER (pokey_pct→100): Pokey is up at the meteor with the cops (the meteor_crater landmark is NW — see SCENE). Head that way with goToward + direction, and when an NPC appears in your SCENE, talk(that slot) — Pokey sends you home; READ his line via screen.text(). NAV CAVEAT: outdoor pathfinding to the crater is UNRELIABLE right now (navTo jams on the hill's SW-detour; a dense named route is the next fix) — do your best toward the crater direction. Do NOT enter the Minch house (px>=0x1C00 scores 0).
  3. HEAD HOME (pokey_knock_pct→80): Pokey sent you home — head for the ness_home_door landmark (see SCENE direction). When the SCENE shows an entity named 'mom', talk('mom') and she sends you inside (her line: "[redacted dialogue]"). Then head upstairs and walk onto your bed. (Same outdoor nav caveat.) Knock caps at 80 for now (the sleep→KNOCK beat isn't bot-triggerable yet — hold, don't thrash).
...
proc buildStateSummary*(ctx: policy.PolicyContext): string =
  ...
  let objective =
    if tgPct < 100:
      ...
      "GET TO THE CRATER. Pokey is up at the meteor with the cops — the meteor_crater landmark is to your NW (see SCENE). Head that way (goToward toward it / the intermediate landmarks) and when ANY NPC appears in your SCENE, talk(that slot) — Pokey sends you home; READ his line via screen.text(). NAV CAVEAT (honest): outdoor pathfinding to the crater is currently UNRELIABLE — navTo jams on the hill's SW-detour, so you may stall; a dense named route is the next fix. Do the best you can toward the crater direction; don't enter the Minch house. Target pokey_pct=100."
    elif pokeyKnockPct < 80:
      "HEAD HOME. Pokey sent you home — the ness_home_door landmark is your target (see SCENE direction); head there. When the SCENE shows an entity named 'mom', talk('mom') and she sends you inside (her line: \"[redacted dialogue]\"). Once inside, head upstairs and walk onto your bed. (Same outdoor nav caveat as above — the long leg may stall until the dense route lands.) Target pokey_knock_pct=80."
    else:
