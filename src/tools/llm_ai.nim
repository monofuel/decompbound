...
SETTING: You are Ness. A METEOR just crashed into the hills above Onett in the middle of the night — the whole town's rattled. This is EarthBound: it's funny, it's weird, lean into it. FOLLOW >>> CURRENT_OBJECTIVE at the top of the RICH STATE — it's authoritative and ADVANCES as you finish milestones, so never keep re-doing a finished one (once pokey_pct=100, STOP talking to Pokey and move on). The story ladder, each step unlocking the next:
  ...
  2. GET TO THE CRATER (pokey_pct→100): Pokey is up at the meteor with the cops. Travel by LANDMARK (the SCENE lists each with a direction) — chain goToward('onett_road') → goToward('hill_climb') → goToward('crater_ridge') → goToward('meteor_crater'); one far target jams on the hill, so step through them. When an NPC appears in your SCENE at the crater, talk(that slot) — Pokey sends you home; READ his line via screen.text(). Do NOT enter the Minch house (px>=0x1C00 scores 0).
  3. HEAD HOME (pokey_knock_pct→80): Pokey sent you home — chain goToward('crater_ridge') → goToward('hill_climb') → goToward('onett_road') → goToward('ness_home_door'). Your MOM is at the door — when the SCENE shows an entity named 'mom', talk('mom') and she sends you inside (her line: "[redacted dialogue]"). Then head upstairs and walk onto your bed. Knock caps at 80 for now (the sleep→KNOCK beat isn't bot-triggerable yet — hold, don't thrash).
...
proc buildStateSummary*(ctx: policy.PolicyContext): string =
  ...
  let objective =
    if tgPct < 100:
      ...
      "GET TO THE CRATER. Your neighbor Pokey is up at the meteor with the cops. Travel by LANDMARK (the SCENE lists them with directions) — chain goToward through them since one far target jams on the hill: goToward('onett_road') → goToward('hill_climb') → goToward('crater_ridge') → goToward('meteor_crater'). When an NPC shows up in your SCENE at the crater, talk(that slot) — Pokey sends you home; READ his line via screen.text(). Don't enter the Minch house. Target pokey_pct=100."
    elif pokeyKnockPct < 80:
      "HEAD HOME. Pokey sent you home — chain goToward back: goToward('crater_ridge') → goToward('hill_climb') → goToward('onett_road') → goToward('ness_home_door'). Your MOM is at the door — when the SCENE shows an entity named 'mom', talk('mom') and she sends you inside (her line: \"[redacted dialogue]\"). Once inside, head upstairs and walk onto your bed. Target pokey_knock_pct=80."
    else:
