...
const NamedRoutesLua* = """
-- Named dense routes live in the skill library (engine), not the LLM policy.
-- Policies call followRoute(name) with no coordinates; the trail points are
-- engine-held reachability chains (same shape as followTrail).
-- onett_to_crater: door -> meteor talk spot. Points = exact PokeyVisitPolicy
-- / PokeyKnockPolicy outdoor trail (TAS 20260709-225653 dense samples) —
-- do not invent alternate coords; this is the verified crater corridor.
-- Requires: followTrail, escapeMenu.
-- Returns true while following; false when the route end is reached (or name
-- unknown / empty).

_namedRoutes = _namedRoutes or {
  onett_to_crater = {
    {x=0x0A60, y=0x0158},
    {x=0x0A4B, y=0x0169},
    {x=0x0A4A, y=0x0192},
  ...
  -- crater_to_onett: meteor talk spot -> house door. The HOME leg. Points =
  -- exact PokeyKnockPolicy _knockTrail (TAS 20260709-225653 dense reverse
  -- samples). NOT a naive reverse of onett_to_crater — following that backwards
  -- drives greedily at point 1 (the door) and local-mins on the climb detour
  -- (verified knock=10 stall 2026-07-17). This is its OWN forward trail (index
  -- 1 = crater), so followTrail seats it symmetrically, same as the up leg.
  crater_to_onett = {
    {x=0x0858, y=0x0128},
    {x=0x0854, y=0x0105},
    {x=0x082F, y=0x0112},
...
function followRoute(name)
  ...
  if name == nil or _namedRoutes == nil then
    return false
  end
  local pts = _namedRoutes[name]
