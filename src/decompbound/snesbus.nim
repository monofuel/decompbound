...
type
  ...
  SnesBus* = ref object
    ...
    fixedColorG*: uint8
    fixedColorB*: uint8
    ## Mode 7 general-purpose multiply ($211B/$211C → $2134-$2136).
    ## EB fills the intro red-snow HDMA distortion table via
    ## M7A (16-bit) * last M7B byte (signed 8-bit) → 24-bit MPY. Without this
    ## every scroll entry is 0 and the intro shows a coherent Giygas face.
    m7a: uint16
    m7b: uint16
    m7MulLatch: uint8
...
proc translatedVramAddr*(snes: SnesBus): uint16 =
  ## Effective VRAM word address for $2118/$2119 after VMAIN bits 3-2 remapping.
  ##
  ## Without this, event/dialogue DMA (Mr. Saturn talk, inn wake-up, sanctuary
  ## visions, etc.) that programs a non-zero translation deposits tiles in the
  ## wrong VRAM words → total graphics garbage / hung black screens. trans=0
  ## is identity (intro/title paths unchanged).
  ##
...
proc ppuPortWrite(snes: SnesBus, offset: uint32, value: uint8): bool =
  ...
  of 0x211C:
    ...
    true
  of 0x2118:
    let index = snes.translatedVramAddr().int
