...
##
## Usage:
##   nim r src/probes/probe_pc_coverage.nim <rom> [--states DIR]... [--replays DIR]
##          [--frames N] [--out FILE]
##
...
proc main() =
  ...
  while i <= paramCount():
    ...
    inc i
  if romPath.len == 0:
    echo "Usage: nim r src/probes/probe_pc_coverage.nim <rom> [--states DIR] [--replays DIR] [--frames N] [--out FILE]"
    quit(1)
