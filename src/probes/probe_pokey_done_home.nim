...
proc main() =
  ...
  for f in 1 .. 8000:
    ...
    if k > maxK: maxK = k
    if f mod 500 == 0 or maxK >= 80:
      echo fmt"f={f} joy=0x{ctx.joy1:04X} pos=(0x{readU16(snes,WorldXBase+i):04X},0x{readU16(snes,WorldYBase+i):04X}) knock={k} maxK={maxK} win1={readU8(snes,0x8654):#04x} 9877={readU8(snes,0x9877):#04x}"
    if maxK >= 80: break
