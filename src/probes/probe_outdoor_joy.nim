...
proc main() =
  ...
  for f in 1 .. 8000:
    ...
    if pokeyPercent(snes) >= 100 and not home:
      ...
      home = true
      echo "handoff"
    if home and f >= 5000 and f mod 100 == 0:
      echo fmt"f={f} joy=0x{ctx.joy1:04X} pos=(0x{readU16(snes,WorldXBase+i):04X},0x{readU16(snes,WorldYBase+i):04X}) knock={pokeyKnockPercent(snes)}"
main()
