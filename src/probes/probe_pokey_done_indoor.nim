...
proc main() =
  ...
  for f in 1 .. 6000:
    ...
    if wasOut and px >= 0x1C00:
      wasOut = false
      echo fmt"ENTER indoor f={f} pos=(0x{px:04X},0x{py:04X}) knock={k} 9877={readU8(snes,0x9877):#04x}"
    if not wasOut and (f mod 50 == 0 or maxK >= 80):
      echo fmt"f={f} joy=0x{ctx.joy1:04X} pos=(0x{px:04X},0x{py:04X}) knock={k} max={maxK} room={currentRoomLabel(snes)}"
    if maxK >= 80: break
