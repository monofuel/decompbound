  ...
  for f in 1 .. 12000:
    ...
    if home and pokeyKnockPercent(snes) >= 70:
      for e in 1 .. 50:
        ...
        snes.joy1 = ctx.joy1
        policy.stepOneFrame(snes, c, img)
        echo fmt"f={f+e} joy=0x{ctx.joy1:04X} pos=(0x{readU16(snes,WorldXBase+i):04X},0x{readU16(snes,WorldYBase+i):04X}) win0={readU8(snes,0x8650):#04x} win1={readU8(snes,0x8654):#04x}"
      break
main()
